...
if(empty($_POST['nome_admin']) || empty($_POST['nome_cliente']) || empty($_POST['nome_funcionario']) || empty($_POST['valor_nota']) || empty($_POST['motivo']) || empty($_POST['obs'])){
  header('Location: ../../createOrdem.php');
  exit();
}
   ...
   <body>
    ...
    <main id="main">
     ...
     <table>
       ...
       <tr>
         <td><span>Prestador: </span><?php $sql = "SELECT user_nome FROM user_Employee WHERE user_nome = '$nome_funcionario'"; $query = mysqli_query($conn, $sql);$result = mysqli_fetch_array($query); echo "$result[0]"; ?></td>
         <td><span>Telefone: </span><?php $sql = "SELECT user_tel FROM user_Employee WHERE user_nome = '$nome_funcionario'"; $query = mysqli_query($conn, $sql); $result = mysqli_fetch_array($query); echo "$result[0]"; ?></td>
         <td><span>Função: </span><?php $sql = "SELECT user_funcao FROM user_Employee WHERE user_nome = '$nome_funcionario'"; $query = mysqli_query($conn, $sql); $result = mysqli_fetch_array($query); echo "$result[0]"; ?></td>
       </tr>

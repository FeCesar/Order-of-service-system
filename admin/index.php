<?php
  ...
  // Linha original do problema 113 - 146
  // $sql = "SELECT * FROM user_employee";
  // $query = mysqli_query($conn, $sql);
  //   while($dados = mysqli_fetch_assoc($query)){
  //     $sql = "SELECT sum(cha_valor) FROM chamadas WHERE cha_nome_funcionario = '" . $dados['user_nome'] . "'";
  //      $query = mysqli_query($conn, $sql);
      ...
      function drawChart() {
        ...
        <?php
         ...
         ?>
          ...
          ['<?php echo $dados['user_nome'] ?>', <?php $sql = "SELECT sum(cha_valor) FROM chamadas WHERE cha_nome_funcionario = '" . $dados['user_nome'] . "'"; $query = mysqli_query($conn, $sql); $result = mysqli_fetch_assoc($query); echo $result[0];?>],

        <?php }; ?>
        ]);

        var options = {'title':'Dinheiro Ganho Das Ordens de Serviços'};
